...
public class bullet : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (GetComponent<bullet>().enabled == true)
        {
            ...
            {
				if (effect == 3)
					collision.gameObject.GetComponent<OgTank>().ReturnTank().GetComponent<tank>().DecreaseHp(damage, true);
				else
					collision.gameObject.GetComponent<OgTank>().ReturnTank().GetComponent<tank>().DecreaseHp(damage, false);
                if (effect == 3)
                {
