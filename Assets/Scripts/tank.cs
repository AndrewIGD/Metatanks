...
public class tank : MonoBehaviour
{
    ...
    private void BurnEffect()
    {
        if (burn == 1)
        {
            ...
            hp = Mathf.Lerp(inithp, inithp - 10 * damageMultiplier, t14);
    ...
    private void Idle()
    {
        if (runninganim == 0 && ableToRun == 1 && jumps == 1)
        {
            while (t10 < 1)
            {
    ...
    public void DecreaseHp(float damag, bool takeDmg)
    {
        if (takeDmg == false)
        {
            if (ableToTakeDMG == 0)
            {
                hp = hp - damag * damageMultiplier;
                ableToTakeDMG = 1;
                Invoke("ATTD", 0.05f);
                hpBar.transform.localScale = new Vector3(100 * hp / maxhp / 100, hpBar.transform.localScale.y, hpBar.transform.localScale.z);
                if (hp <= 0)
    ...
    private void Executables()
    {
        ...
        else if (GetComponent<Rigidbody2D>().velocity.y <= -1.5f && foot1.GetComponent<feet1>().GetColl() == 1 && foot2.GetComponent<feet2>().GetColl() == 1)
        {
            if (jumped2 == 0)
            {
                foot1pos = foot1.transform.localPosition;
